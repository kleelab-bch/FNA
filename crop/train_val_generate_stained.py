...
def augment_data(imgs,msks,augmentation_factor):
    ...
    train = np.zeros((augmentation_factor*batch_size, imgs.shape[1], imgs.shape[2], imgs.shape[3])).astype('uint8')
    mask = np.zeros((augmentation_factor*batch_size, imgs.shape[1], msks.shape[2], msks.shape[3])).astype('uint8')
    
    print('Data Generating...')
    for samples in range(augmentation_factor): 
...
class data_generate:
    def __init__(self, path, input_size, output_size, random_seed, saved_folder, img_format = '.png', rand_crop_num = 200, root = '../../DataSet_label/', img_folder = '/img/', mask_folder = '/mask/'):
        self.path = path
    ...
    def get_img_mask_list(self, root_image_path):
        ...
        img_size = 1024
        images = np.ndarray((total_number, img_size, img_size, 3), dtype=np.uint8)
        masks = np.ndarray((total_number, img_size, img_size, 3), dtype=np.uint8)
        
        for i in range(len(mask_list)):
            print('mask:', i)
            mask = cv2.imread(mask_list[i], cv2.IMREAD_COLOR)
            masks[i] = mask[:img_size,:img_size,:]
            
        for i in range(len(img_list)):
            print('img:', i)
            img = cv2.imread(img_list[i], cv2.IMREAD_COLOR)
            images[i] = img[:img_size,:img_size,:]
    ...
    def split_val(self, inputs, imgsTrain_indices, imgsVal_indices, imgsTest_indices):
        ...
        return train_0, val_0, test_0, train_1, val_1, test_1
    #==================================================================================
    #==================================================================================
    ...
    def crop(self, saved_file_path, root_image_path, crop_type):
        ...
        imgsTrain, imgsVal, imgsTest, msksTrain, msksVal, msksTest = self.split_val([image, mask], imgsTrain_index, imgsVal_index, imgsTest_index)
        
        if crop_type == 'train':
            return imgsTrain, msksTrain
        
        elif crop_type == 'valid':
            return imgsVal, msksVal
